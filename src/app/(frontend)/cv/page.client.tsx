...
export default async function CVPageClient({ cv }: CVPageClientProps) {
    ...
    return (
      <div key={formation.id} className="w-full mb-4">
        ...
        <p className="font-italic">
          {formation.organisme}
          {formation.location && formation.organisme
            ? ', ' + formation.location
            : formation.location}
          {(formation.location || formation.organisme) && dateString.length
            ? ' - ' + dateString
            : dateString}
        </p>
        <ul className="list-disc ml-8">
          {formation.description?.map((li) => {
            return <li key={li.id}>{li.label}</li>
          })}
        </ul>
      </div>
    )
  ...
  const expositionsRender = expositions.map((exposition) => {
    ...
    return (
      <div key={exposition.id} className="w-full mb-4">
        ...
        <p className="font-italic">
          {exposition.expositionName}
          {exposition.location && exposition.expositionName
            ? ', ' + exposition.location
            : exposition.location}
          {(exposition.location || exposition.expositionName) && dateString.length
            ? ' - ' + dateString
            : dateString}
        </p>
        <ul className="list-disc ml-8">
  ...
  const artisticActivitiesRender = artisticActivities.map((artisticActivity) => {
    ...
    return (
      <div key={artisticActivity.id} className="w-full mb-4">
        ...
        <p className="font-italic">
          {artisticActivity.location}
          {artisticActivity.location && dateString.length ? ' - ' + dateString : dateString}
        </p>
        <ul className="list-disc ml-8">
  ...
  return (
    <div className="max-w-dvw flex flex-col md:flex-row md:gap-[150px] my-8 p-4 md:mx-[100px]">
      <div className="md:w-[600px] md:min-w-[600px] flex flex-col gap-4">
        <h1 className="block md:hidden text-(--color-lila) text-center text-[64px] leading-[60px] font-[Rockwell_Condensed] uppercase">
          {cv.perso.firstname} {cv.perso.lastname}
        </h1>
        <h2 className="block md:hidden text-[40px] text-center text-black uppercase mb-4">
          {cv.perso.job}
        </h2>
        ...
        <div className="md:w-[600px] rounded-[30px] bg-(--color-green) text-[24px] md:text-[40px] flex flex-col items-center p-4 gap-8 md:gap-16">
          <h2 className="text-[30px] md:text-[48px] leading-[30px] md:leading-[48px]">
            Mes coordonnées
          </h2>
          ...
          <a
            href="/contact"
            className=" text-black px-8 py-4 hover:bg-(--color-dark-cream) bg-(--color-cream) transition-all duration-300 rounded-[15px]"
          >
            Me contacter
          </a>
        </div>
      ...
      <div className="w-full flex flex-col items-center text-black mt-4 md:mt-0">
        <h1 className="hidden md:block text-(--color-lila) text-[64px] leading-[60px] font-[Rockwell_Condensed] uppercase">
          {cv.perso.firstname} {cv.perso.lastname}
        </h1>
        ...
        {expositionsRender}
        <h2 className="text-[40px] leading-[40px] text-center uppercase my-4">
