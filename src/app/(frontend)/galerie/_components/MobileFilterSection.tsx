...
export default function MobileFilterSection({
  ...
  const handleResetFilter = () => {
    ...
    Array.prototype.forEach.call(els, (val: HTMLElement) => {
  ...
  const filtersRender = filters.map((filter) => {
    return (
      <>
        ...
        <div
          id={filter.type}
          className="text-[16px] leading-[30px] flex flex-row flex-wrap gap-1 transition-all duration-300 w-full filters"
        >
          {filter.items.map((item) => {
            return (
              <button
                ...
                className="px-2 rounded text-center min-w-max border-[1px] border-(--color-green) hover:bg-(--color-dark-cream) cursor-pointer filters-btn"
              >
                {item.name}
              </button>
            )
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-4 left-4 bg-(--color-green) w-max py-2 px-3 [&>svg]:h-[24px]! rounded"
      >
        <FontAwesomeIcon icon={faFilter} />
      </button>
      <div
        id="filters-mobile"
        className="fixed bottom-0 left-0 flex flex-col bg-(--color-cream) text-black w-full h-0 overflow-hidden text-[24px] leading-[24px] [&.open]:h-full transition-all duration-300"
      >
        ...
        <div className="w-full p-2 flex gap-2">
          <button
            ...
            className="bg-(--color-dark-cream) text-(--color-green) rounded cursor-pointer leading-[40px] w-full"
          >
            Reinitialiser
          </button>
          <button
            onClick={() => handleOpenFilter()}
            className="bg-(--color-green) text-(--color-cream) rounded cursor-pointer leading-[40px] w-full"
          >
            Appliquer
          </button>
        </div>
